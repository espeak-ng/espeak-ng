...
ESPEAK_DATA_PATH=$(pwd)/build build/src/espeak-ng -v en "Hello world"

# Phoneme output (useful for debugging pronunciation)
ESPEAK_DATA_PATH=$(pwd)/build build/src/espeak-ng -xq -v en "Hello world"
```
...
Tests run from the build directory with `ESPEAK_DATA_PATH` pointing to the build root (which contains `espeak-ng-data/`).
