...
JNICALL Java_com_reecedunn_espeak_SpeechSynthesis_nativeCreate(
    ...
    int sampleRate = espeak_Initialize(AUDIO_OUTPUT_SYNCHRONOUS, BUFFER_SIZE_IN_MILLISECONDS,
                                       c_path, 0);
...
JNICALL Java_com_reecedunn_espeak_SpeechSynthesis_nativeSetVoiceByName(
    ...
    switch (result) {
        ...
        case EE_INTERNAL_ERROR:
            LOGE("espeak_SetVoiceByName: internal error.");
            break;
        case EE_BUFFER_FULL:
            LOGE("espeak_SetVoiceByName: buffer full.");
            break;
        case EE_NOT_FOUND:
            LOGE("espeak_SetVoiceByName: not found.");
            break;
    }
...
JNICALL Java_com_reecedunn_espeak_SpeechSynthesis_nativeSetVoiceByProperties(
    ...
    switch (result) {
        case EE_OK:
            return JNI_TRUE;
        case EE_INTERNAL_ERROR:
            LOGE("espeak_SetVoiceByProperties: internal error.");
            break;
        case EE_BUFFER_FULL:
            LOGE("espeak_SetVoiceByProperties: buffer full.");
            break;
        case EE_NOT_FOUND:
            LOGE("espeak_SetVoiceByProperties: not found.");
            break;
    }
...
JNICALL Java_com_reecedunn_espeak_SpeechSynthesis_nativeSetParameter(
    ...
    switch (result) {
        case EE_OK:
            return JNI_TRUE;
        case EE_INTERNAL_ERROR:
            LOGE("espeak_SetParameter: internal error.");
            break;
        case EE_BUFFER_FULL:
            LOGE("espeak_SetParameter: buffer full.");
            break;
        case EE_NOT_FOUND:
            LOGE("espeak_SetParameter: not found.");
            break;
    }
...
JNICALL Java_com_reecedunn_espeak_SpeechSynthesis_nativeSetPunctuationCharacters(
    ...
    switch (result) {
        case EE_OK:
            return JNI_TRUE;
        case EE_INTERNAL_ERROR:
            LOGE("espeak_SetPunctuationList: internal error.");
            break;
        case EE_BUFFER_FULL:
            LOGE("espeak_SetPunctuationList: buffer full.");
            break;
        case EE_NOT_FOUND:
            LOGE("espeak_SetPunctuationList: not found.");
            break;
    }
...
JNICALL Java_com_reecedunn_espeak_SpeechSynthesis_nativeSynthesize(
    ...
    const espeak_ERROR result = espeak_Synth(c_text, strlen(c_text), 0,  // position
                                             ...
                                             isSsml ? espeakCHARS_UTF8 |
                                                      espeakSSML // UTF-8 encoded SSML
    ...
    switch (result) {
        case EE_OK:
            return JNI_TRUE;
        case EE_INTERNAL_ERROR:
            LOGE("espeak_Synth: internal error.");
            break;
        case EE_BUFFER_FULL:
            LOGE("espeak_Synth: buffer full.");
            break;
        case EE_NOT_FOUND:
            LOGE("espeak_Synth: not found.");
            break;
    }
